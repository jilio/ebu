...
func TestDirectHandlerCalls(t *testing.T) {
	...
	t.Run("func_context_any_handler", func(t *testing.T) {
		...
		called := false
		var handler interface{} = func(ctx context.Context, event any) {
			called = true
			if s, ok := event.(string); !ok || s != "test" {
				t.Errorf("expected string 'test', got %v", event)
			}
	...
	t.Run("func_context_any_error_handler", func(t *testing.T) {
		...
		called := false
		var handler interface{} = func(ctx context.Context, event any) error {
			called = true
			if s, ok := event.(string); !ok || s != "test" {
				t.Errorf("expected string 'test', got %v", event)
			}
	...
	t.Run("Handler_T_interface", func(t *testing.T) {
		...
		called := false
		var handler Handler[string] = func(event string) {
			called = true
			if event != "test" {
	...
	t.Run("ContextHandler_T_interface", func(t *testing.T) {
		...
		called := false
		var handler ContextHandler[string] = func(ctx context.Context, event string) {
			called = true
			if event != "test" {
				t.Errorf("expected 'test', got %s", event)
			}
