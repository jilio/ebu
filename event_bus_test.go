...
func TestAsyncSequentialHandlerContextCancelled(t *testing.T) {
	bus := New()
	var handlerCalled int32
	// Buffered: up to 5 events may be published, and the test only receives
	// once. An unbuffered channel would block the remaining async handlers
	// forever and hang bus.Wait().
